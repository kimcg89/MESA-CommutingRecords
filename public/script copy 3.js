...
const firebaseConfig = {
  apiKey: "AIzaSyDKGe_Asc5qezw3-QIpgF7VkdGikodqk5w",
  authDomain: "mesa-commutingrecords.firebaseapp.com",
  projectId: "mesa-commutingrecords",
  storageBucket: "mesa-commutingrecords.firebasestorage.app",
  messagingSenderId: "168419938550",
  appId: "1:168419938550:web:204a3ac6734c5fb6d85cd7",
};
...
async function saveRecordToFirestore(record) {
  try {
    ...
    console.log("Record successfully added to Firestore:", record);
  } catch (error) {
  ...
  navigator.geolocation.getCurrentPosition(
    async (position) => {
      ...
      if (!isClockedIn) {
          ...
          start: [
            {
              ...
              gps: `위도: ${latitude.toFixed(6)}, 경도: ${longitude.toFixed(
                6
              )}`,
            },
          ],
        ...
        records.push(record);
        await saveRecordToFirestore(record); // Firestore에 저장
        updateRecords();
      } else {
        // 퇴근 처리
        ...
        await saveRecordToFirestore(lastRecord); // Firestore에 저장
        updateRecords();
        clockInTime = null;
      }
    },
...
//   if (user) {
//     console.log("사용자가 로그인 중:", user.email);
//   } else {
