...
function updateRecords() {
  ...
  recordList.innerHTML = records
    .map((record) => {
      if (record.type === "퇴근") {
        console.log(
          `<li>${record.type}: ${record.time} (근무 시간: ${record.duration})GPS: ${record.gps}</li>`
        ); //퇴근 시간 및 gps 기록
        return `<li>${record.type}: ${record.time} (근무 시간: ${record.duration})</li>`;
      }
      console.log(`${record.type}: ${record.time}GPS: ${record.gps}`); //출근 시간 및 gps 기록
      return `<li>${record.type}: ${record.time}</li>`;
    })
    .join("");
}
...
function generateCalendar(year, month) {
  ...
  for (let date = 1; date <= lastDate; date++) {
    ...
    if (holiday) {
      ...
      cell.innerHTML = `<span>${date}</span><br><small>${holiday.name}</small>`;
    }

    // 기념일 확인 및 표시
    const mesaday = mesadays.find((m) => m.date === formattedDate);
    if (mesaday) {
      cell.innerHTML += `<br><small>${mesaday.name}</small>`;
    }

    // 출퇴근 기록 표시
    const dayRecords = records.filter((record) => {
      // record.time에서 날짜 부분 추출
      const match = record.time.match(/^(\d{4})\.\s*(\d{1,2})\.\s*(\d{1,2})/);
      const recordDate = match
    ...
    dayRecords.forEach((record) => {
      ...
      marker.classList.add(record.type === "출근" ? "marker-in" : "marker-out"); // 출근/퇴근 구분
      marker.innerText = record.type === "출근" ? "출" : "퇴";

      console.log(`Marker 생성됨: ${marker.innerText}`); // Marker 생성 확인

      if (record.type === "퇴근") {
        const durationBox = document.createElement("div");
        durationBox.classList.add("duration-box");
        durationBox.innerText = record.duration || "";
        cell.appendChild(durationBox);
        console.log(`Duration Box 추가됨: ${durationBox.innerText}`);
      }
